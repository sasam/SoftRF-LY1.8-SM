...
- A `ForceGP` option was implemented in the web interface, so the conversion can be enabled or disabled by the user
...
- Measured current reduction with the screen OFF is approximately 20 mA:
...
- The white status LED is mapped to GPIO18
- Its blink behavior was retuned to reduce visible brightness and average LED on-time
- Behavior on other boards has not been verified
